...
Before beginning research, inspect current protected main, record research_base_sha, and read current AGENTS.md plus docs/research-lane-bounded-execution.md, docs/stage3a-regular-season-site-research.md, docs/stage3a0-local-only-contract.md, docs/stage3a1-source-fanout-contract.md, docs/stage3a3-neutral-tranche-contract.md, and docs/shared-reference-authority.md. Follow the current repository research policies those documents reference. Repository policy controls over copied handoff wording.
...
Stage 3A has required substages (3A-0 through 3A-4). Treat those substages as real owner-facing stop boundaries. Stage 3A-0 is a strict local-only mechanical pass: no public-web research, external source discovery, institutional archive browsing, or broad GitHub/code search is authorized. Stage 3A-1 also has a strict source-fanout contract: a bounded row queue does not authorize broad web discovery; use the repository's authoritative source hierarchy and converge when those paths are exhausted. Stage 3A-3 has a neutral-tranche contract: after substantial research begins, stay within one coherent neutral-research tier and research mode, use a small authoritative source hierarchy, write through before context risk, and do not cross into the next tier in the same turn. For regular-season neutral venue research, modern means 1996-97 through present; 1995-96 and earlier uses the historical proportional-convergence standard, where surviving exact-building debt is nonblocking after systematic/high-yield exhaustion. Evidence classes inside 3A-1 through 3A-3 are research units, not automatic owner-facing stops: bundle multiple small classes that use the same research mode into meaningful safe tranches.
...
Before acting, inspect current protected main and read current AGENTS.md plus docs/research-lane-bounded-execution.md, docs/stage3a-regular-season-site-research.md, docs/stage3a0-local-only-contract.md, docs/stage3a1-source-fanout-contract.md, docs/stage3a3-neutral-tranche-contract.md, and docs/shared-reference-authority.md.
...
If the earliest incomplete work is Stage 3A-1, resume only the serialized H/A/N queue and obey the source-fanout contract. Define coherent evidence classes first. Use accepted project evidence, then the target-school institutional source family, then the obvious opponent institutional source family, and at most one specific high-yield authoritative fallback family when justified. Generic web search is discovery-only. Do not fan out through unrelated websites or repeat already-exhausted paths; when the bounded hierarchy is exhausted, preserve/terminalize the row as current policy requires and move on.

If the earliest incomplete work is Stage 3A-3, resume only the exact serialized neutral tier/tranche. Do not reconstruct failed-turn scratch research. For regular-season neutral venue research, classify 1996-97+ as modern and 1995-96-and-earlier as historical. If protected-main policy advanced across the historical-neutral standard, mechanically reclassify only the unresolved residual from authoritative rows; do not reopen or downgrade already-supported exact venues. Finish any incomplete modern recurring/one-off work under the strong exact-venue standard. Then combine the remaining 1995-96-and-earlier regular-season neutral residual into one location-enrichment pass: use accepted project evidence plus the obvious systematic event/host/participant opportunity, preserve event/city/state, accept an exact building only when the same evidence directly supplies it, and once city/state is established do not continue solely to recover the building. Do not apply this shortcut to postseason; Stage 3B NCAA/NIT/conference-tournament/other postseason exact venues remain exhaustive research obligations.
