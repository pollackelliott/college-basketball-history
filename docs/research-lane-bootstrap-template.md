...
If the earliest incomplete work is Stage 3A-3, resume only the exact serialized neutral tier/tranche. Do not reconstruct failed-turn scratch research. Stay within one neutral tier for the turn: modern recurring, modern one-offs, historical recurring, or historical one-offs. For regular-season neutral venue research, classify 1996-97+ as modern and 1995-96-and-earlier as historical. If protected-main policy advanced across a cutoff change, mechanically reclassify only the unresolved residual from authoritative rows; do not reopen or downgrade already-supported exact venues. Use accepted project evidence, event/host institutional evidence, target-school or obvious participant institutional evidence, and at most one specific high-yield authoritative fallback family when justified. Generic web search is discovery-only. Write accepted findings through and serialize the exact remaining tiers before crossing into a materially different research mode; do not continue an exact-source tail chase after the bounded hierarchy is exhausted.
