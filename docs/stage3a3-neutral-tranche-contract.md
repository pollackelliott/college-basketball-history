# Stage 3A-3 Neutral-Tranche and Source-Fanout Contract

- **Status:** Controlling Research-lane execution policy
- **Applies to:** Stage 3A-3 regular-season NEUTRAL venue research
- **Approved:** 2026-09-23 after Virginia Stage 3A-3 expanded across multiple modern/historical neutral families and many source paths in one turn
- **Purpose:** prevent a large neutral population from becoming one open-ended venue-research campaign
...
- **population order**; and
- **source-fanout discipline**.

## 2. Required Stage 3A-3 execution tiers
...
## 3. Tranche sizing inside a tier
...
This contract does not weaken the anti-inference rule.

Do not assign a physical venue from:

- city alone;
- event custom alone;
- another edition's venue;
- opponent home arena;
- nearby games;
- chronology alone.

A systematic event-family assignment is valid only when the evidence actually covers the exact edition/class being applied.

## 8. Write-through and recovery

After a meaningful tranche:

- write accepted venue/city/state/provenance findings through into the authoritative row-level Stage 3A state;
- serialize every researched-but-unresolved row/family;
- serialize exact remaining populations by tier;
...
- resume only the exact serialized unfinished tier/tranche;
- do not repeat completed tiers;
- do not repeat exhausted source families;
- do not reconstruct unsupported conclusions from failed-turn scratch artifacts;
- do not broaden into the next tier until the current tier is durably brought to disposition.

## 9. Stop conditions

Stop the current turn when any of these occurs:

- the active tier is fully brought to disposition;
- a meaningful related-family tranche is complete and the next work requires a materially different research mode;
- the source hierarchy for the active family/class is exhausted;
- a genuine contradiction requires narrow isolation;
- execution/context risk is becoming material.

Do **not** continue into a different tier just because time/context appears available.

## 10. Review rule

Flag a Stage 3A-3 execution if it:

- crosses multiple neutral-research tiers after substantial research in one turn;
- opens many unrelated source families for one event/row;
