...
class ResearchRoundPolicyTests(unittest.TestCase):
            ...
            [
                {
                    ...
                    "season_label": "1979-1980",
                    "game_date": "1980-03-06",
    ...
    def test_blank_round_is_valid_when_no_honest_controlled_mapping_exists(self):
    ...
    def test_invalid_populated_round_is_still_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self.make_package(
                root,
                round_name="Round of 48",
                raw_text="NCAA Tournament game",
            )
            report = research_portfolio_report(root, school_key="test")
            ...
            self.assertTrue(
                any(
                    "invalid NCAA curated_postseason_round" in error
